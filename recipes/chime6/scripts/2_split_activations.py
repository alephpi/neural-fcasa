...
CORPUS_ROOT = SCRIPT_ROOT.parent / "chime6corpus"
MODE_TO_SPLIT = {
    "tr": "train",
    "cv": "dev",
    "tt": "eval",
...
def resolve_data_roots(mode, rttm_root=None, audio_root=None):
    ...
    return rttm_base / split_name, audio_base / split_name
...
def split_data_one(scenario_name, duration, stepsize, dst_path, rttm_root, audio_root):
    ...
    wav_file1 = audio_root / f"{scenario}_U01.CH1.wav"
    wav_file2 = audio_root / f"{scenario}_U02.CH1.wav"

    wav_duration1 = sf.info(wav_file1).duration
    wav_duration2 = sf.info(wav_file2).duration
    wav_duration = min(wav_duration1, wav_duration2)
