...
def split_data_one(scenario_file, duration, stepsize, dst_path):
    scenario_basename = scenario_file.stem

    # we only use U01 and U02 to combine a 8-channel signal
    suffixes = ["U01.CH1", "U01.CH2", "U01.CH3", "U01.CH4", "U02.CH1", "U02.CH2", "U02.CH3", "U02.CH4"]
    wavs = []
    N1 = sf.info(scenario_file.parent / f"{scenario_basename}_U01.CH1.wav").frames
    N2 = sf.info(scenario_file.parent / f"{scenario_basename}_U02.CH1.wav").frames
    N = min(N1, N2)
    sr = 16000
    for suffix in suffixes:
        wav_filename = scenario_file.parent / f"{scenario_basename}_{suffix}.wav"
        wav, sr = sf.read(wav_filename, N)
        wavs.append(wav)
    wav = np.stack(wavs, axis=1)  # (T, 8), like what soundfile reads for multi-channel audio
...
def split_data(args, unk_args):
    ...
    for scenario_basename in data_split[args.mode]:
        if args.mode == "tr":
            dataset_path_ = dataset_path / "train"
        elif args.mode == "cv":
            dataset_path_ = dataset_path / "dev"
        elif args.mode == "tt":
            dataset_path_ = dataset_path / "eval"
        else:
            raise ValueError(f"unknown mode: {args.mode}")
        scenario_file_list.append(dataset_path_/scenario_basename)
    print(scenario_file_list)
    # return

    dst_path = Path(f"./chime6corpus/processed_data/{args.mode}") / "mix"
